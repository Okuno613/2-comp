...
double dV_d(double V_d,double V_s,double h_d,double n_d,double p_d){
  double m_inf_d=1.0/(1+exp(-(V_d-V12_d)/k_m_inf_d));
  return( g_na_d* m_inf_d*m_inf_d *h_d * (V_na - V_d) + g_dr_d * n_d*n_d *p_d *(V_k -V_d)+ g_c/(1.0-kappa)*(V_s-V_d) + g_leak *(V_l - V_d));
}
      
double dh_d(double h_d, double V_d){
  double h_inf_d=1.0/(1.0+exp(-(V_d-V12_h_d)/k_h_d));
  return( (h_inf_d - h_d) /tau_h_d );
}

double dn_d(double n_d, double V_d){
  double n_inf_d=1.0/(1.0+exp(-(V_d-V12_n_d)/k_n_d));
  return( (n_inf_d - n_d) /tau_n_d );
}

double dp_d(double p_d, double V_d){
  double p_inf_d=1.0/(1.0+exp(-(V_d-V12_p_d)/k_p_d));
...
double dV_s(double V_s, double inp, double V_d,double n_s){
  ...
  return (inp + g_na_s * m_inf_s*m_inf_s  * (1.0-n_s )* (V_na - V_s) +g_dr_s * n_s*n_s *(V_k -V_s)+ g_c/kappa*(V_d-V_s) +g_leak *(V_l-V_s));
}

double dn_s(double n_s, double V_s){
  double n_inf_s=1.0/(1.0+exp(-(V_s-V12_s)/k_m_inf_s));
...
void calv(double *V_s, double *n_s,double *V_d,double *h_d, double *n_d,double *p_d,  int *spike_s,int *spike_d, double *inp, double t,int *spikecnt_s,int *spikecnt_d,int *count_s,int *count_d,double *THl,FILE *fp1,FILE *fp2,FILE *fp3,FILE *fp4,FILE *fp5)
{
  ...
  for(int i=0;i<NUM;i++){
    ...
    //    inp[i]=I0lif*sin(2.0*M_PI*t/100);
...
void Simulation::sim()
{
    ...
    fp2=fopen("double_doi_cpp_Vd_moved.txt","w");
    fp3=fopen("Vs.txt","w");
    fp4=fopen("Vd.txt","w");
    fp5=fopen("pd.txt","w");
